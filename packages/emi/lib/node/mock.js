...
// src/node/mock.ts
function getMockPaths({ appData }) {
  const mockDir = appData.paths.absMockPath;
  const mockTsFiles = glob.sync("**/*.ts", {
    cwd: mockDir
  });
  const absMockPaths = mockTsFiles.map((file) => import_path2.default.join(mockDir, file));
  return absMockPaths;
}
function getMockConfig({ appData }) {
  const mockOutDir = import_path2.default.join(
  ...
  const absMockPaths = mockJsFiles.map((file) => import_path2.default.join(mockOutDir, file));
  const config = absMockPaths.reduce((p, c) => {
    const config2 = require(c).default;
    delete require.cache[c];
    return { ...p, ...config2 };
  }, {});
  const formatedConfig = Object.entries(config).reduce((p, c) => {
    const [req, handler] = c;
